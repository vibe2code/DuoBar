...
enum MenuBarIconSize {
    static let preferenceKey = "reduoBar.menuBarIconScale"
    static let legacyPreferenceKey = "duoBar.menuBarIconScale"
    ...
    static func storedScale(in defaults: UserDefaults = .standard) -> Double {
        let rawValue = (defaults.object(forKey: preferenceKey) as? NSNumber)?.doubleValue
            ?? (defaults.object(forKey: legacyPreferenceKey) as? NSNumber)?.doubleValue
